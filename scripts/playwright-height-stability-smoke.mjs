...
function assertHeightStability(report) {
  ...
  if (report.smooth.maxDelta >= report.baseline.maxDelta)
    failures.push(`smooth.maxDelta expected < baseline.maxDelta, got ${report.smooth.maxDelta} >= ${report.baseline.maxDelta}`)
  if (report.constrainedSmooth.maxDelta >= report.constrainedBaseline.maxDelta)
    failures.push(`constrainedSmooth.maxDelta expected < constrainedBaseline.maxDelta, got ${report.constrainedSmooth.maxDelta} >= ${report.constrainedBaseline.maxDelta}`)
  if (!report.smooth.transition.includes('height'))
    failures.push(`smooth.transition expected to include height, got ${report.smooth.transition}`)
  if (!report.constrainedSmooth.transition.includes('height'))
    failures.push(`constrainedSmooth.transition expected to include height, got ${report.constrainedSmooth.transition}`)
